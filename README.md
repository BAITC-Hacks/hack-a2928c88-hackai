...
| **Второе мнение NVIDIA** | Отдельная проверка тех же исходных полей без ответа первой модели; результат хранится отдельно | Проверено локально в mock, ключа NVIDIA пока нет |
| **Вопросы по карточке** | Студент задаёт публичный вопрос (не больше 5 в сутки на карточку) и видит похожие. Бизнес отвечает, правки сохраняют историю, ответ можно перенести в поле карточки. Модерация, подписка, уведомления внутри приложения | Локально: тесты и браузерный сценарий |
| **Рекомендательное письмо** | После проекта капитан задаёт роли, участники их подтверждают, бизнес заполняет опросник. Каждый участник получает личный PDF с QR-проверкой; есть перевыпуск, отзыв и жалоба на факты. Письмо на русском, казахском или английском | Локально: тесты и браузерный сценарий. [Подробно](docs/COMMUNITY.md) |
| **Казахский язык** | Экраны вопросов и писем на русском и казахском | Локально |
...
|---|---|
| Публичный сайт | Работает. В Edge пройден полный сценарий с реальным OpenAI (`mode=live`, `provider=openai`): 0/100/80/100, публикация, отклик, ручной выбор, +10 один раз. Модель тогда была `gpt-4.1-mini` |
| Что уже на Render | Редизайн, «Что помешает начать?», прогноз позиции, эндпоинты приёмки этапа и ТЗ (проверено по `/openapi.json`) |
| Ещё не на Render | GPT-5.5, второе мнение NVIDIA, вопросы по карточке, рекомендательные письма. Нужны новый образ и `OPENAI_MODEL=gpt-5.5` в Environment, см. [docs/DEPLOY.md](docs/DEPLOY.md) |
| GPT-5.5 | Реальные локальные запросы на синтетике прошли для вопросов и цитат, проверки карточки и ТЗ |
| NVIDIA | Код и mock проверены. Ключа нет, реальный вызов не проверен |
| Текущая версия локально | 156 тестов; браузерные сценарии `smoke_e2e`, `smoke_questions`, `smoke_project_letters` пройдены |
| Постоянный диск на Render и доступность весь период судейства | Не подтверждены. Данные на публичном сайте считайте временными; QR-проверка писем требует постоянной базы и стабильного адреса |
